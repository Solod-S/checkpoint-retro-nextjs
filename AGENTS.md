...
## 3.1. Infrastructure
- Приложение должно запускаться напрямую на host OS.
- Локальная БД — PostgreSQL system service.
- Production target — Ubuntu VPS + Nginx + PM2 + PostgreSQL.
- Не добавлять новый infrastructure/runtime layer без отдельного ADR и явного запроса пользователя.
